...
fn err(id: Value, code: i32, message: impl Into<String>) -> RpcResponse {
    RpcResponse { id, result: None, error: Some(RpcError { code, message: message.into() }) }
}
...
pub async fn run_stdio(registry: ConnectionRegistry) -> anyhow::Result<()> {
    ...
    while let Some(line) = reader.next_line().await? {
        ...
        let response = match req {
            Ok(r) => handle_request(r, &registry).await.unwrap_or_else(|e| err(json!(null), -32000, e.to_string())),
            Err(e) => {
                err(json!(null), -32700, format!("Parse error: {}", e))
            }
        };
...
async fn handle_request(req: RpcRequest, registry: &ConnectionRegistry) -> anyhow::Result<RpcResponse> {
    match req.method.as_str() {
        "register_connection" => {
            #[derive(Deserialize)]
            struct P { id: String, engine: String, dsn: String }
            let p: P = serde_json::from_value(req.params)?;
            let engine = parse_engine(&p.engine)?;
            registry.register(ConnectionConfig { id: p.id, engine, dsn: p.dsn, default_schemas: None, include_system: Some(false) }).await?;
            Ok(ok(req.id, json!({"ok": true})))
        }
        "list_connections" => {
            let items = registry.list().await.into_iter().map(|(id, engine)| json!({"id": id, "engine": engine_to_str(engine)})).collect::<Vec<_>>();
        ...
        "health_check" => {
            ...
            struct P { id: String }
            let p: P = serde_json::from_value(req.params)?;
            let Some(handle) = registry.get(&p.id).await else { return Ok(err(req.id, 404, "Unknown connectionId")); };
            let v: i32 = sqlx::query_scalar("SELECT 1").fetch_one(&handle.pool).await?;
        ...
        "introspect_all" => {
            ...
            struct P {
                ...
                #[serde(rename = "withIndexes")] with_indexes: Option<bool>,
                #[serde(rename = "withRoutines")] with_routines: Option<bool>,
                #[serde(rename = "withTypes")] with_types: Option<bool>,
                #[serde(rename = "withTriggers")] with_triggers: Option<bool>,
                page: Option<u32>,
                #[serde(rename = "pageSize")] page_size: Option<u32>,
                format: Option<String>,
            }
            let p: P = serde_json::from_value(req.params)?;
            let Some(handle) = registry.get(&p.connection_id).await else { return Ok(err(req.id, 404, "Unknown connectionId")); };
            match handle.engine {
                DatabaseEngine::Postgres => {
                    ...
                    if is_paged {
                        ...
                        let slice = if (start as usize) < pairs.len() { &pairs[start as usize..end as usize] } else { &[][..] };

                        use std::collections::BTreeMap;
                        let mut by_schema: BTreeMap<String, Vec<crate::core::models::TableDetail>> = BTreeMap::new();
                        ...
                        for (schema_name, tables) in by_schema {
                            ...
                            schemas_vec.push(crate::core::models::Schema { name: schema_name, tables, views });
                        }
                        let db = crate::core::models::DatabaseSchema { database: dbname, schemas: schemas_vec };
                        ...
                        if want_markdown {
                            ...
                            if p.with_triggers.unwrap_or(false) {
                                ...
                                if !tg.is_empty() { md.push_str("\n触发器：\n"); for t in tg { md.push_str(&format!("- {}.{} [{} {}]\n", t.schema, t.name, t.timing, t.events.join("/"))); } }
                            }
                            if p.with_routines.unwrap_or(false) {
                                let rt = pg.list_routines(None, None).await.unwrap_or_default();
                                if !rt.is_empty() { md.push_str("\n例程：\n"); for r in rt { md.push_str(&format!("- {}.{} ({:?})\n", r.schema, r.name, r.kind)); } }
                            }
                            if p.with_types.unwrap_or(false) {
                                let tp = pg.list_types(None, None).await.unwrap_or_default();
                                if !tp.is_empty() { md.push_str("\n类型：\n"); for t in tp { md.push_str(&format!("- {}.{} ({:?})\n", t.schema, t.name, t.kind)); } }
                            }
                            return Ok(ok(req.id, json!({
                                "markdown": md,
                    ...
                    if let Some(s) = &p.schemas { let mut s2 = s.clone(); s2.sort(); key_parts.push(format!("schemas={}", s2.join(","))); }
                    ...
                    if !want_markdown {
                        if let Some(v) = handle.cache.get(&cache_key).await { return Ok(ok(req.id, v)); }
                    }

                    let out = pg.introspect_database(IntrospectOptions {
                        schemas: p.schemas,
        ...
        "refresh_cache" => {
            #[derive(Deserialize)]
            struct P { id: String, scope: Option<String>, schema: Option<String>, table: Option<String> }
            let p: P = serde_json::from_value(req.params)?;
            let Some(handle) = registry.get(&p.id).await else { return Ok(err(req.id, 404, "Unknown connectionId")); };
        ...
        "list_triggers" => {
            #[derive(Deserialize)]
            struct P { #[serde(rename = "connectionId")] connection_id: String, schema: Option<String> }
            let p: P = serde_json::from_value(req.params)?;
            let Some(handle) = registry.get(&p.connection_id).await else { return Ok(err(req.id, 404, "Unknown connectionId")); };
            let pg = PostgresIntrospector::new(handle.pool.clone());
        ...
        "list_routines" => {
            #[derive(Deserialize)]
            struct P { #[serde(rename = "connectionId")] connection_id: String, schema: Option<String>, kind: Option<String> }
            let p: P = serde_json::from_value(req.params)?;
            let Some(handle) = registry.get(&p.connection_id).await else { return Ok(err(req.id, 404, "Unknown connectionId")); };
            let kind = match p.kind.as_deref() { Some("function") => Some(RoutineKind::Function), Some("procedure") => Some(RoutineKind::Procedure), Some("aggregate") => Some(RoutineKind::Aggregate), _ => None };
            let pg = PostgresIntrospector::new(handle.pool.clone());
        ...
        "list_types" => {
            ...
            let p: P = serde_json::from_value(req.params)?;
            let Some(handle) = registry.get(&p.connection_id).await else { return Ok(err(req.id, 404, "Unknown connectionId")); };
            let kind = match p.kind.as_deref() { Some("enum") => Some(TypeKind::Enum), Some("domain") => Some(TypeKind::Domain), Some("composite") => Some(TypeKind::Composite), Some("base") => Some(TypeKind::Base), _ => None };
            let pg = PostgresIntrospector::new(handle.pool.clone());
        ...
        "render_er" => {
            #[derive(Deserialize)]
            struct P { #[serde(rename = "connectionId")] connection_id: String, schemas: Option<Vec<String>> }
            let p: P = serde_json::from_value(req.params)?;
            let Some(handle) = registry.get(&p.connection_id).await else { return Ok(err(req.id, 404, "Unknown connectionId")); };
            let pg = PostgresIntrospector::new(handle.pool.clone());
            let db = pg.introspect_database(crate::core::models::IntrospectOptions {
                schemas: p.schemas,
                with_indexes: false,
                with_routines: false,
                with_types: false,
        ...
        "get_table" => {
            #[derive(Deserialize)]
            struct P { #[serde(rename = "connectionId")] connection_id: String, schema: String, table: String, format: Option<String> }
            let p: P = serde_json::from_value(req.params)?;
            let Some(handle) = registry.get(&p.connection_id).await else { return Ok(err(req.id, 404, "Unknown connectionId")); };
            match handle.engine {
                DatabaseEngine::Postgres => {
...
fn engine_to_str(e: DatabaseEngine) -> &'static str {
    match e { DatabaseEngine::Postgres => "postgres" }
}
