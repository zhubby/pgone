...
fn render_table_line(t: &TableDetail) -> String {
    let mut s = String::new();
    if let Some(c) = &t.comment { s.push_str(&format!("- 表 `{}`（{}）\n", t.name, c)); } else { s.push_str(&format!("- 表 `{}`\n", t.name)); }
    ...
    if let Some(pk) = &t.primary_key { s.push_str(&format!("  - 主键：({})\n", pk.columns.join(", "))) }
    for fk in &t.foreign_keys { s.push_str(&format!("  - 外键：({}) → {}({})\n", fk.columns.join(", "), fk.ref_table, fk.ref_columns.join(", "))) }
    for idx in &t.indexes {
        let inc = if idx.include.is_empty() { String::new() } else { format!(" INCLUDE ({})", idx.include.join(", ")) };
        s.push_str(&format!("  - 索引：{} ON ({}){}{}\n", idx.name, idx.columns.join(", "), if idx.unique { " UNIQUE" } else { "" }, inc))
    }
    s
}
