...
impl McpContext {
    ...
    async fn get_session(&self, dbconfig_id: &str) -> anyhow::Result<Session> {
        ...
        Session::new(&config.dsn).await.map_err(|e| anyhow::anyhow!("Failed to create session: {}", e))
    }
}
...
impl PgoneMcpServer {
    ...
    fn create_tool(name: &'static str, description: &'static str, schema: Value) -> Tool {
        let schema_obj = schema.as_object().expect("Schema must be an object").clone();
        Tool {
            name: name.into(),
...
impl ServerHandler for PgoneMcpServer {
            ...
            tools: vec![
                ...
                Self::create_tool(
                    ...
                    json!({
                        ...
                        "required": ["dbconfig_id", "schema", "table"]
                    })
                ),
                Self::create_tool(
                    "list_triggers",
    ...
    fn call_tool(
        ...
        async move {
            ...
            let args_value = request.arguments.map(|m| serde_json::Value::Object(m)).unwrap_or_default();

            let result = match tool_name {
                "introspect_all" => PgoneMcpServer { context: context.clone() }.handle_introspect_all(args_value.clone()).await,
                "get_table" => PgoneMcpServer { context: context.clone() }.handle_get_table(args_value.clone()).await,
                "list_triggers" => PgoneMcpServer { context: context.clone() }.handle_list_triggers(args_value.clone()).await,
                "list_routines" => PgoneMcpServer { context: context.clone() }.handle_list_routines(args_value.clone()).await,
                "list_types" => PgoneMcpServer { context: context.clone() }.handle_list_types(args_value.clone()).await,
                "render_er" => PgoneMcpServer { context: context.clone() }.handle_render_er(args_value.clone()).await,
                "render_dbml" => PgoneMcpServer { context: context.clone() }.handle_render_dbml(args_value.clone()).await,
                "health_check" => PgoneMcpServer { context: context.clone() }.handle_health_check(args_value).await,
            ...
            match result {
                Ok(value) => {
                    ...
                    Ok(CallToolResult {
                        content: vec![rmcp::model::Annotated {
                            raw: rmcp::model::RawContent::Text(rmcp::model::RawTextContent { text, meta: Default::default() }),
                            annotations: Default::default(),
                        }],
                        is_error: Some(false),
                        meta: Default::default(),
                ...
                Err(e) => Ok(CallToolResult {
                    content: vec![rmcp::model::Annotated {
                        raw: rmcp::model::RawContent::Text(rmcp::model::RawTextContent { text: format!("Error: {}", e), meta: Default::default() }),
                        annotations: Default::default(),
                    }],
                    is_error: Some(true),
                    meta: Default::default(),
...
impl PgoneMcpServer {
    ...
    async fn handle_get_table(&self, args: Value) -> anyhow::Result<Value> {
        ...
        let table = introspector.get_table(&params.schema, &params.table).await?;
        let want_markdown = matches!(params.format.as_deref(), Some("markdown"));
    ...
    #[derive(Deserialize)]
        ...
        let routines = introspector.list_routines(params.schema.as_deref(), kind).await?;
    ...
    #[derive(Deserialize)]
        ...
        let types = introspector.list_types(params.schema.as_deref(), kind).await?;
    ...
    #[derive(Deserialize)]
        ...
        // 执行简单查询来检查连接
        let _: String = session.current_database().await.map_err(|e| anyhow::anyhow!("Health check failed: {}", e))?;
        Ok(json!({"ok": true}))
    }
}

/// 运行 STDIO 模式的 MCP 服务器
pub async fn run_stdio(storage_path: std::path::PathBuf) -> anyhow::Result<()> {
