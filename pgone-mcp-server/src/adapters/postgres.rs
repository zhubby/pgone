...
impl DatabaseIntrospector for PostgresIntrospector {
    async fn introspect_database(
        ...
        let mut schemas_map: std::collections::BTreeMap<String, Vec<TableDetail>> =
    ...
    async fn get_table(&self, schema: &str, table: &str) -> anyhow::Result<TableDetail> {
        ...
        for r in fk_rows {
            ...
            let entry = fk_map.entry(cname).or_insert((
                Vec::new(),
        ...
        let foreign_keys: Vec<ForeignKey> = fk_map
            .into_values()
            .map(
                |(cols, (ref_table, ref_cols), on_update, on_delete)| ForeignKey {
                    columns: cols,
                    ref_table,
    ...
    async fn list_views(&self, schema: Option<&str>) -> anyhow::Result<ViewDetailVec> {
        ...
        let mut views: Vec<ViewDetail> = rows
            .into_iter()
            .map(|r| ViewDetail {
                schema: r.get(0),
                name: r.get(1),
        ...
        let mat_rows = if let Some(s) = schema {
            sqlx::query(
                "SELECT schemaname, matviewname, definition FROM pg_matviews WHERE schemaname = $1",
            )
            .bind(s)
            .fetch_all(&self.pool)
            .await?
        } else {
            sqlx::query("SELECT schemaname, matviewname, definition FROM pg_matviews")
                .fetch_all(&self.pool)
                .await?
        };
        for r in mat_rows {
            views.push(ViewDetail {
                schema: r.get(0),
                name: r.get(1),
                definition: r.try_get(2).ok(),
                comment: None,
    ...
    async fn list_triggers(&self, schema: Option<&str>) -> anyhow::Result<Vec<TriggerDetail>> {
        ...
        for r in rows {
            ...
            let entry = map.entry(key).or_insert(TriggerDetail {
                ...
                timing,
                events: Vec::new(),
                function_name: action_stmt
                    .as_ref()
                    .and_then(|s| parse_trigger_function_name(s.as_str())),
            });
    ...
    async fn list_routines(
        ...
        if let Some(_s) = schema {
            sql.push_str(" AND n.nspname = $1");
        }
        ...
        for r in rows {
            ...
            if let Some(ts) = types {
                for (idx, t) in ts.into_iter().enumerate() {
                    ...
                    let mode = modes
                        .as_ref()
                        .and_then(|ms| ms.get(idx).cloned())
                        .and_then(|m| match m.as_str() {
                            "i" => Some(ParamMode::In),
                            "o" => Some(ParamMode::Out),
                            "b" => Some(ParamMode::InOut),
                            "v" => Some(ParamMode::Variadic),
                            "t" => Some(ParamMode::Table),
                            _ => None,
                        });
    ...
    async fn list_types(
        ...
        for r in rows {
            let td = TypeDetail {
                ...
                kind: TypeKind::Enum,
                base_type: None,
                enum_labels: r.try_get(4).ok(),
                definition: None,
                comment: r.try_get(6).ok(),
            };
            if kind
                .as_ref()
                .map(|k| matches!(k, TypeKind::Enum))
                .unwrap_or(true)
            {
                out.push(td);
            }
        ...
        for r in rows {
            ...
            if kind
                .as_ref()
                .map(|k| matches!(k, TypeKind::Domain))
                .unwrap_or(true)
            {
                out.push(td);
            }
        ...
        for r in rows {
            ...
            if kind
                .as_ref()
                .map(|k| matches!(k, TypeKind::Composite))
                .unwrap_or(true)
            {
                out.push(td);
            }
...
fn parse_trigger_function_name(stmt: &str) -> Option<String> {
    ...
    let key = if let Some(pos) = upper.find("EXECUTE FUNCTION") {
        pos + "EXECUTE FUNCTION".len()
    } else if let Some(pos) = upper.find("EXECUTE PROCEDURE") {
