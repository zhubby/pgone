...
use arrow::array::{Array, FixedSizeListArray, Int64Array, StringArray};
...
impl ChatVectorStore {
    ...
    pub async fn insert_batch(&self, records: Vec<ChatVectorRecord>) -> Result<()> {
        ...
        let batches = RecordBatchIterator::new(
            ...
            Arc::new(crate::table::create_schema(self.vector_dimension)?),
        );
    ...
    pub async fn search(
        ...
        if let Some(role) = &options.role {
            ...
            filters.push(format!("role = '{}'", role_str));
        }

        if let Some(min_ts) = options.min_timestamp {
            filters.push(format!("timestamp >= {}", min_ts));
        }

        if let Some(max_ts) = options.max_timestamp {
            filters.push(format!("timestamp <= {}", max_ts));
        }
        ...
        for batch in results {
            ...
            let embedding_model_idx = schema
                .column_with_name("embedding_model")
                .map(|(idx, _)| idx);
