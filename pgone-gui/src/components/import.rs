...
impl ImportWindow {
    pub fn ui(&mut self, ui: &mut egui::Ui, db_manager: &mut DbManager) {
        ui.vertical(|ui| {
                        ...
                        .show(ui, |ui| {
                            ...
                            for (idx, result) in self.results.iter().enumerate() {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    if result.success {
                                        ui.label(format!("✓ {}", 
                                            result.rows_affected.map(|r| format!("影响 {} 行", r))
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button(if self.is_importing { "取消" } else { "关闭" }).clicked() {
                    if !self.is_importing {
                        // 如果不在导入中，重置所有状态
                        ...
                        match current_section {
                            ...
                            _ => {
                                ...
                                if upper.starts_with("CREATE") || upper.starts_with("ALTER") 
                                    || upper.starts_with("DROP") || upper.starts_with("COMMENT") {
    ...
    fn start_import(&mut self, db_manager: &mut DbManager) {
        ...
        self.import_status = format!("准备导入... (DDL: {}, DML: {})", ddl_statements.len(), dml_statements.len());
        self.results.clear();
            ...
            if let Some(result) = promise.ready() {
                match result {
                    Ok(results) => {
                        ...
                        self.import_status = format!("导入完成！成功: {}, 失败: {}", success_count, fail_count);
