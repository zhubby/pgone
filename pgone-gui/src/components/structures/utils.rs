...
/// Build column type string from ColumnDetail
...
/// Generate complete table DDL statement
pub fn generate_table_ddl(
    ...
    for col in &table_detail.columns {
        let mut col_def = format!(
            ...
            build_column_type_from_detail(col)
        );
    ...
    if !table_detail
        .columns
        .iter()
        .any(|c| c.comment.is_some() && !c.comment.as_ref().unwrap().trim().is_empty())
    {
        // If there are no column comments, remove the last newline
        if ddl.ends_with("\n\n") {
            ddl.pop();
        }
    } else {
    ...
    // Add indexes (excluding primary key indexes, as primary keys automatically create indexes)
    for idx in indexes {
        ...
        if let Some(ref definition) = idx.definition {
            // If definition exists, use it directly
            ddl.push_str(&format!(
                "CREATE {}INDEX {} ON {}.{} {};\n",
            ...
            // Otherwise, build using column names
            ddl.push_str(&format!(
                "CREATE {}INDEX {} ON {}.{} ({})",
                if idx.unique { "UNIQUE " } else { "" },
                quote_ident(&idx.name),
...
/// Generate table DML (INSERT) statements
pub fn generate_table_dml(
    ...
    // Batch generate INSERT statements, 100 per batch
