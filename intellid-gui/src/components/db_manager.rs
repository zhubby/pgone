...
impl DbManager {
    ...
    pub fn ui_db_config(&mut self, app: &mut crate::IntelliGuiApp, ui: &mut egui::Ui) {
        ...
        if let Some(storage) = &self.storage {
            ...
            for cfg in list {
                ...
                if resp.double_clicked() { to_switch = Some(cfg.id.clone()); }
            }
        } else {
        ...
        if let Some(target) = to_switch {
            ...
            egui::Window::new("Switch Database Config").open(&mut open).collapsible(false).resizable(false).show(ui.ctx(), |ui| {
                ...
                ui.horizontal(|ui| {
                    if ui.button("Confirm").clicked() { self.active_db_config_id = Some(target.clone()); }
    ...
    pub fn ui_add_db_window(&mut self, _app: &mut crate::IntelliGuiApp, ctx: &egui::Context) {
        if self.show_add_db {
            let mut open = true;
    ...
    pub fn ui_manage_db_window(&mut self, _app: &mut crate::IntelliGuiApp, ctx: &egui::Context) {
        if self.show_manage_db {
            ...
            egui::Window::new("Databases").open(&mut open).show(ctx, |ui| {
                ...
                } else { ui.label("Storage not ready"); }
            });
