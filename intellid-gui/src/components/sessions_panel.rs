...
impl SessionsPanel {
    pub fn ui(&mut self, ctxs: &mut SessionsCtx, ui: &mut egui::Ui) {
        ...
        if ui.button("+ New Session").clicked() {
            ...
            ctxs.state.sessions.push(Session { id, title: format!("Session {}", id), messages: Vec::new(), db: DbConfig { engine: "postgres".to_string(), dsn: String::new() } });
            ...
            if let Some(storage) = &ctxs.db.storage {
                ...
                let _ = ctxs.db.rt.block_on(async { storage.create_session(&sess).await });
            }
        }
        ui.separator();
        let items: Vec<(usize, String)> = ctxs.state.sessions.iter().enumerate().map(|(i, s)| (i, s.title.clone())).collect();
        for (idx, title) in items {
            ui.horizontal(|ui| {
                let selected = idx == ctxs.state.current_index;
                if ui.selectable_label(selected, &title).clicked() { ctxs.state.current_index = idx; }
            ...
            if self.renaming_index == Some(idx) {
                ui.horizontal(|ui| {
                    ...
                    if ui.button("Save").clicked() || (resp.lost_focus() && press_enter) {
                        if let Some(s) = ctxs.state.sessions.get_mut(idx) { s.title = self.rename_buffer.trim().to_string(); }
