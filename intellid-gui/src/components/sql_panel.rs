...
impl Default for SqlPanel {
    fn default() -> Self { Self { sql_input: String::new(), sql_error: None, query_columns: vec![], query_rows: vec![] } }
}
...
impl SqlPanel {
    ...
    pub fn ui_results(&mut self, ui: &mut egui::Ui) {
        ui.heading("Results");
        ui.separator();
        if self.query_columns.is_empty() { ui.label("No results"); return; }
        ...
        egui::ScrollArea::vertical().auto_shrink([false; 2]).show(ui, |ui| {
            egui::Grid::new("results_table").striped(true).spacing([8.0, 4.0]).show(ui, |ui| {
                for col in &self.query_columns { ui.strong(col); }
                ui.end_row();
                for row in &self.query_rows { for cell in row { ui.label(cell); } ui.end_row(); }
                let data_rows = self.query_rows.len();
                if data_rows < max_visible_rows {
                    let empty_rows_needed = max_visible_rows - data_rows;
                    for _ in 0..empty_rows_needed { for _ in &self.query_columns { ui.add_space(0.0); } ui.end_row(); }
                }
            });
        });
    }
    ...
    pub fn run_sql(&mut self, app: &mut crate::IntelliGuiApp) {
        ...
        let res: Result<(Vec<String>, Vec<Vec<String>>), String> = rt.block_on(async move {
            ...
            if let Some(first) = rows.get(0) { for c in first.columns() { cols.push(c.name().to_string()); } }
            for row in rows.into_iter().take(100) {
                let mut r: Vec<String> = Vec::new();
                let n = if cols.is_empty() { row.len() } else { cols.len() };
                for i in 0..n { r.push(crate::sql::format_cell(&row, i)); }
                data.push(r);
            }
            Ok((cols, data))
        });
        match res { Ok((cols, rows)) => { self.query_columns = cols; self.query_rows = rows; } Err(e) => { self.sql_error = Some(e); } }
    }
